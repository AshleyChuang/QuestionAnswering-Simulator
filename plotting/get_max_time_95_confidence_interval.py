...
for filename in os.listdir(path1):
	if filename.endswith('.txt'):
		...
		for line in fin:
			...
			bottle_max.append(float(response_time[-1])/3600.0)
		bottle.append(bottle_max)
...
for filename in os.listdir(path2):
	if filename.endswith('.txt'):
		...
		for line in fin:
			...
			response_time = lineArr[4].split(" ")
			response_time.sort(key=float)
			iask_max.append(float(response_time[-1])/3600.0)
		iask.append(iask_max)
sos = []
...
for filename in os.listdir(path3):
	if filename.endswith('.txt'):
		...
		for line in fin:
			...
			response_time = lineArr[4].split(" ")
			response_time.sort(key=float)
			sos_max.append(float(response_time[-1])/3600.0)
		sos.append(sos_max)
bottle_mean = []
bottle_std = []
for i in bottle:
...
sos_std = []
for i in sos:
	sos_mean.append(reduce(lambda x, y: x + y, i) / len(i))
