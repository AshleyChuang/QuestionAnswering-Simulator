...
for line in fin_active_time:
    ...
    if time_range[lineArr[0]][-1][-1] == 25 and time_range[lineArr[0]][0][0] == 1:
        time_range[lineArr[0]][0].extend(time_range[lineArr[0]][-1])
...
def match_interval(interval, intervals):
    ...
    for _interval in intervals:
        if len(set(interval) & set(_interval)) > 0:
...
def find_shortpath_by_cost(net, time_range, nodes, check, s, t):
    ...
    while (remain_nodes):
        ...
        for node in net[min_node]:
            ...
            if not matched:
                penalty = time_diff(interval[min_node], _interval)*3600
    ...
    return path_node[::-1], path_cost[::-1], cost[t]
...
#        print min_node, interval[min_node]
    ...
    return path_node[::-1], path_original_cost[::-1], path_cost[::-1], original_cost[t], cost[t]
...
##    print '_____________________________________________________________________'
    ...
    if int(st_time) > int(st_end):
        time_range[ nodes[int(source)] ] = range(1, int(st_end)+1) + range(int(st_time), 25)
    else:
...
for ind in final_ans:
    ...
    shortest_path, original_cost_path, cost_path, original_cost, cost = find_shortpath(net, time_range, nodes, check, nodes[int(source)], nodes[ind[0]])
...
##        print ""
##   
##    shortest_path, cost_path, cost = find_shortpath_by_cost(net, time_range, nodes, check, nodes[int(source)], nodes[ind[0]])
##
##    if cost == -1:
