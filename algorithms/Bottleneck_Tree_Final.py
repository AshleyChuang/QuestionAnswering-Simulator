...
##        print "Total path Delay: -"
        ...
        filtered_result.append( nodes[int(source)] + "," + nodes[ind[0]] + "," + "[" + ',' .join(shortest_path) + "]" + "," + str(cost) )
        filtered_score.append( cost )

for ans_idx in sorted(range(len(filtered_score)), key=lambda k: filtered_score[k])[:int(req_num)]:
    print filtered_result[ans_idx]
